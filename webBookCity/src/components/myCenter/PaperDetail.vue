        ...
        <div class="box-con mt20 global-box">
            <div class="order-code">
                <div class="order-code-left">
                    ...
                    <h2 v-if="orderdata.Status==3&&!orderdata.IsPaid" >等待付款</h2>
                    <h2 v-if="orderdata.Status==3&&orderdata.IsPaid" >等待发货</h2>
                    <h2 v-if="orderdata.Status==4">已发货</h2>
...
<!--                    <p v-if="orderdata.Status!=1">该订单会为您保留24.0小时（从下单之日算起），24.0小时之后如果还未付款，系统将自动取消该订单。</p>-->
                    <div class='tempo'>
                        ...
                        <img v-if="orderdata.Status==1||(orderdata.Status==3&&orderdata.IsPaid)||orderdata.Status==4" src="../../assets/fukuan2.png">
                        <img v-if="orderdata.Status==3&&!orderdata.IsPaid" src="../../assets/fukuan.png">
                        <img v-if="orderdata.Status==1||(orderdata.Status==3&&orderdata.IsPaid)||orderdata.Status==4" src="../../assets/jindu1.png">
                        <img v-if="orderdata.Status==3&&!orderdata.IsPaid" src="../../assets/jindu2.png">
                        
                        <img v-if="orderdata.Status==1||orderdata.Status==4" src="../../assets/chuku2.png">
                        <img v-if="(orderdata.Status==3&&!orderdata.IsPaid)||(orderdata.Status==3&&orderdata.IsPaid)" src="../../assets/chuku.png">
                        <img v-if="orderdata.Status==1||orderdata.Status==4" src="../../assets/jindu1.png">
                        <img v-if="(orderdata.Status==3&&!orderdata.IsPaid)||(orderdata.Status==3&&orderdata.IsPaid)" src="../../assets/jindu2.png">
...
<!--
                        ...
                        <span>等待收货</span>
-->
                        <span style="margin-left: 485px;"></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="box-con mt20 global-box">
            <div class="buyer-info">
               <ul>
                    ...
                    <li>
                        <h6>付款信息</h6>
                        <p>付款方式：<span>网上支付</span></p>
                    </li>
                    <li>
                        <h6>发票信息</h6>
                        <p v-if='orderdata.ExtendData.OrderInvoiceId'>发票类型：<span>{{invoicedata.Type==1?'个人':'单位'}}</span></p>
                        <p v-if='orderdata.ExtendData.OrderInvoiceId'>发票抬头：<span>{{invoicedata.Title?invoicedata.Title:'无'}}</span></p>
                        <p v-if='orderdata.ExtendData.OrderInvoiceId'>发票内容：<span>{{invoicedata.InvoiceContentType==1?'详情':'图书'}}</span></p>
                        <p v-if='!orderdata.ExtendData.OrderInvoiceId'>无</p>
                    </li>
               </ul>
            </div>
        </div>
...
<el-dialog title="确认收货" :visible.sync="logisticsDialogVisible" width="544px" center>
    ...
    <el-button @click="confirm($route.query.orderId)">确 定</el-button>
  </span>
</el-dialog>
...
<script>
    ...
    export default {
        data() {
            return {
                orderdata:{
                    ExtendData:{NickName:'',OrderAddressInfo:''}
                },
                tableData: [],
                invoicedata:'',
        ...
        methods: {
            ...
            //获取详情
            getInvoice() {
                this.$http.get("/Invoice/Detail", {
                        params: {
                            id:this.orderdata.ExtendData.OrderInvoiceId
                        }
                    })
                    .then((res) => {
                        if (res.data.Success) {
...
/*                            text-align: center;*/
                ...
                &>.goods-info {
                    .goods-info-detail {
                        ...
                        white-space: nowrap;
                        .divimg1{ border: 1px solid #eee; float: left; width: 80px; height: 112px; margin-right: 10px; } .divimg { float: left; width: 78px; height: 110px; }
