          ...
          <div style="overflow: hidden;" v-if="addressList.length>0">
            ...
            <div v-if="showList" v-loading="getAddressListLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(256, 256, 256, 0.8)">
              <div class="address-list" v-for="(item,index) in addressList">
                <p>{{index+1}}.{{item.Reciver}}
                  <span style="margin-left: 10px;">
                    {{item.Country}}{{item.Province}}{{item.DetailedAddress}}{{item.PostCode&&item.PostCode!=''?','+item.PostCode:''}},{{item.MobilePhone}}
                  </span>
                </p>
                <p style="color:#999">
                  <el-checkbox style="margin:0" @change="changeDefaultAd(item.Id)" v-model="item.checked">设置为默认收货地址</el-checkbox>
                </p>
                <span class="change-span">
                  <el-button class="change-btn" @click="addAddressModal(2,item)">修改</el-button>
                  <el-button class="change-btn" @click="changeAddressFn(item)">更换</el-button>
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="goods-info">
          ...
          <el-table :data="tableData" style="width: 100%;margin-top: 20px" :header-row-class-name="tableRowClassName">
            ...
            <el-table-column align="center" label="商品类型" width="170">
              <template slot-scope='scope'>
                <p>{{scope.row.MediaType?returnType(scope.row.MediaType):'-'}}</p>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="oldPrice" label="价格" width="170">
              <template slot-scope='scope'>
                <p>&yen;{{returnTypePrice(scope.row.MediaType,scope.row)}}</p>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="小计" width="120">
              <template slot-scope='scope'>
                <p>&yen;{{formatPrice(returnTypePrice(scope.row.MediaType,scope.row)*scope.row.Count,2)}}</p>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    getTotalMoney() {
      if (this.tableData.length > 0) {
        this.tableData.forEach((item, index) => {
          this.TotalMoney = this.TotalMoney + parseFloat(this.returnTypePrice(item.MediaType,item) * item.Count, 2)
